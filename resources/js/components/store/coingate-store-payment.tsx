...
interface CoinGateStorePaymentProps {
  orderId: number;
  orderTotal: number;
  currency: string;
  store: any;
...
export function CoinGateStorePayment({
  orderId,
  orderTotal,
  currency,
  store,
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    router.post(
      generateStoreUrl('store.coingate.payment', store),
      {
        order_id: orderId
      },
      {
        onError: (errors) => {
          console.error('CoinGate payment error:', errors);
          setIsProcessing(false);
        },
        onFinish: () => {
          // Don't reset processing state here as we're redirecting
        }
      }
    );
  };
